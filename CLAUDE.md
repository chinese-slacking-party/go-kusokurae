...
**`sm/` — Core game engine (cgo wrapper around C).** The card game "Kusokurae" is implemented in `sm.c` (deck, dealing, trick-taking, scoring, busting). `sm.go` wraps it via cgo using Go structs with identical memory layout to the C structs — `unsafe.Pointer` casts convert between them without serialization. Go replaces the C PRNG with `math/rand` via a callback. State transition callbacks bridge from C back to Go through `callbackMap`. Key types: `GameConfig`, `GameState`, `Player`, `Card`, `RoundState`.
